...
class ClientUI(wx.Frame):
    ...
    def __init__(self,c,parent=None,id=-1,title=''):
        ...
        self.user_list = wx.ListBox(self.panel_user,style=wx.ALIGN_CENTER|wx.LB_SINGLE,size = (260,600))
    ...
    def OnClick_sendButton(self,event):
        ...
        if text:
            ...
            if self.logicClient.sendMsg(11,text,2,{"time":theTime}):
                #将消息显示到窗口
                title = "我 " + theTime
                message = " " + text
        ...
        while True:
            try:
                ...
                if msg['type'] == 2:
                    ...
                    title = "shuifade!"
                    text = msg['data']
                    self.ui.addMessage(title,text,1)
            except:
                break
        sock.close()
    ...
    def syncServer(self):
        while True:
            try:
                ...
                sendSock.connect((self.serverIP, self.serverPort))
                if self.status == 0:
                    msg= {"type":1,"from":self.me,"ip":self.hostIP,"port":self.hostPort,"data":"login"}
                elif self.status >= 0:
                    msg= {"type":1,"from":self.me,"data":"hello"}
